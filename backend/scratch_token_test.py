...
def count_tokens():
    ...
    data = [
        ...
        {
            "name": "Osc A Level",
            "min": float("-inf") if False else -70.0,
            "max": 0.0,
            "value": -12.0,
            "quantized": False,
        },
    ...
    res_json = client.models.count_tokens(
        model="models/gemini-3.1-flash-lite-preview", contents=json_str
    )

    res_toon = client.models.count_tokens(
        model="models/gemini-3.1-flash-lite-preview", contents=toon_str
    )
