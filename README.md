...
- **Session State Drawer**: A slide-out panel that dynamically tracks your active project's BPM, Key, and Track count, while managing new session initializations.
- **Hierarchical Cost Tracking**: Real-time tracking of API token consumption, specifically breaking down the exact cost of your most recently executed prompt.
- **Vertical Fullscreen UX**: The application launches perfectly spanning your entire monitor height to mimic Ableton's vertical browser flow.
...
- **Dynamic Rack Architecture**: Directory-driven custom rack cataloging. Drop any custom Ableton `.adg` device into your User Library, and the background worker automatically parses the macros and generates semantic JSON bindings via `gemini-3.1-flash-lite`, effortlessly exposing new instruments to the AI without writing code.
