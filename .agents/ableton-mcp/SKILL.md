...
- **Backend Architecture:** A uvicorn/FastAPI server (`backend/main.py`) running on `127.0.0.1:8000` exposing REST endpoints and wrapping the `CreativePlannerAgent`. The frontend is a secure Electron desktop application (`frontend/`) that communicates via NDJSON streaming for real-time status updates.
